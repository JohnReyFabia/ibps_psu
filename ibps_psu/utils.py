...
from burnout_assessment.models import Program
import csv
import json
...
def insert_row(d):
    ...
    if User.objects.filter(email=d["Email Address"]).exists():
        ...
        form = StudentForm(
            {
                ...
                "program": Program.objects.get(program_name=d["Program"]),
                "password": "1234",
        ...
        if form.is_valid():
            ...
            raise Exception(f"Error creating user: {form.errors} with data: {d}")
...
for i, data in enumerate(d, start=1):
    student, new = insert_row(data)
    if new:
        print(f"{i}) student: {student} created successfully")
    else:
        print(f"{i}) student: {student} already exists")
